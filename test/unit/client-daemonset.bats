...
@test "client/DaemonSet: hostPath data directory set to /opt/consul by default" {
  cd `chart_dir`
  local actual=$(helm template \
      -x templates/client-daemonset.yaml  \
      . | tee /dev/stderr |
...
@test "client/DaemonSet: hostPath data directory can be configured" {
  cd `chart_dir`
  local actual=$(helm template \
      -x templates/client-daemonset.yaml  \
      --set 'client.dataDirectoryHostPath=/another/dir' \
      . | tee /dev/stderr |
      yq '.spec.template.spec.volumes[0].hostPath.path == "/another/dir"' | tee /dev/stderr)
  [ "${actual}" = "true" ]
}

@test "client/DaemonSet: hostPath data directory can be disabled" {
  cd `chart_dir`

  # Test that hostPath is set to null.
  local actual=$(helm template \
      -x templates/client-daemonset.yaml  \
      --set 'client.dataDirectoryHostPath=' \
      . | tee /dev/stderr |
